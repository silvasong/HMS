	...
	var handleTable = function() {
		...
		oTable = table
				.dataTable({
					...
					"serverMethod" : "GET",
					"ajaxSource" : rootURI
							+ "admin/checkmanagement/checkOut/list?rand="
							+ Math.random(),
		...
		$('input[name=status]').on('change', function() {
			...
			$('#tf_btn').addClass('hide');
			if (parseInt($(this).val()) == 1) {
				
				$('#tf_btn').removeClass('hide');
			}
		...
		$("#tf_btn").on("click", function(event) {
						...
						function(e) {
							$
									.ajax({
										...
										"type" : "POST",
										"url" : rootURI
												+ "admin/checkmanagement/checkOut/manager/"
												+ selected.join(),
